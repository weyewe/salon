...
=begin
  ...
  6. create sales return based on sales order 5 ( +4) # => 7 
      recover 2 to purchase order 2   ( TO preserve the FIFO )
      recover 1 from purchase order 1 
      recover 1 from migration  
=end
  ...
  context "creating sales order that spans multiple stock_entries (-4)" do
    ...
    context "creating purchase order to create new stock entry (+2)" do 
      ...
      context "creating sales return (-3)" do
        ...
        # sales return == 4
        it "should fill up the stock entries that were deducted by sales order (get the ordering right) FIFO" do
          
          # (@pre_return_used_quantity_second_stock_entry - # used  2
          #      @second_stock_entry.used_quantity) .should ==   @unreturned_sales_quantity  # only returned 2
          #       
          ...
          #      @migration_stock_entry.used_quantity) .should == @migration_stock_entry.quantity # returned  1
          @second_stock_entry.used_quantity.should == 0 
          @second_stock_entry.is_finished.should be_false
          @first_stock_entry.used_quantity.should == 0    # hard coded .. 
          @first_stock_entry.is_finished.should be_false 
          @migration_stock_entry.used_quantity.should ==  1
